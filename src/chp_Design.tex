...
Bei dem Entwurf der GUI sollen die in Abschnitt \ref{sec:QualitätskriterienSW} definierten Grundsätze der Dialoggestaltung aus ISO ISO/IEC 25010 beachtet werden.
 
Die Hauptelemente der Benutzeroberfläche sollen ein Texteditor, ein Projektexplorer, eine Geräteliste sowie eine Konsole bilden. 
Zur Bedienung dieser Hauptelemente soll die GUI über eine Menüleiste, eine Toolbar mit Buttons sowie über Kontextmenüs verfügen.
Die Menüleiste soll alle möglichen Funktionen bereitstellen, die im jeweiligen Bedienungskontext zur Verfügung stehen. 
Den Zugriff auf häufig benötigte Funktionen soll die Toolbar ermöglichen.

Folgende Buttons soll die Toolbar enthalten:
...
Die vier GUI-Hauptelemente sollen innerhalb eines einzelnen Programmfensters dargestellt werden, um dem Benutzer einen zentralen Zugriff auf alle zur Verfügung stehenden Funktionen zu ermöglichen. Als zentrales Element soll in der Mitte des Fensters der Texteditor angeordnet werden, da für die Anzeige von Quellcode viel Platz beansprucht wird. Um innerhalb von größeren Programmskripten zu navigieren, muss der Texteditor scrollbar sein.
Die Konsole soll unterhalb des Texteditors ausgerichtet sein, da ein breites Eingabefeld für Befehle benötigt wird. Da in vertikale Richtung wenig Platz zur Verfügung steht, muss die Textausgabe der Konsole scrollbar sein. 
Die Inhalte des Projektexplorers und der Geräteliste sind in Listenform aufgebaut und benötigen demnach viel Platz in vertikale Richtung. Die Anordnung dieser Elemente rechts und links des Texteditors ermöglichen somit eine optimale Darstellung.

(Vereinzelt Designbegründung nach ISO einfügen)

Abbildung \ref{fig:Mockup} zeigt ein Mockup der Benutzeroberfläche, das die beschriebenen Anordnungen der GUI-Elemente umsetzt.
