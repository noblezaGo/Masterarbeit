...
\begin{figure}[!htbp]
	...
	\caption{Benutzeroberfläche des configurAIDERs}
...
Im Projektexplorer kann ein Projekt erstellt und geöffnet werden, das als Baumstruktur dargestellt wird und aus Ordnern, Pawn-Skriptdateien sowie Pawn-Includedateien besteht. 
Das Projekt repräsentiert das Wurzelelement des Baumes und stellt Ebene 1 der Struktur dar. 
...
WPF stellt die Klasse \texttt{TreeView} zur Verfügung, um Objekte in einer Baumstruktur anzuordnen. 
Objekte in einer \texttt{TreeView} können ein- und aufgeklappt werden, um Kindelemente ein- oder auszublenden. \\
...
\texttt{ItemsSource} wird deshalb an die Property \texttt{Project} gebunden, die in \texttt{ProjectExplorerContainerViewModel} enthalten ist. Diese stellt eine \texttt{ObservableCollection} zur Verfügung, die Objekte vom Typ \texttt{IProjectExplorerItem} enthalten muss. \\
...
Die Klassen \texttt{ProjectViewModel}, \texttt{FolderViewModel}, \texttt{ScriptFileViewModel} sowie \texttt{IncludeFileViewModel} leiten von dieser Basisklasse ab und repräsentieren jeweils ein Baumelement des Projektexplorers.
Die \texttt{ObservableCollection}, die \texttt{Project} zur Verfügung stellt, kann somit ausschließlich Instanzen dieser Klassen enthalten.

Mithilfe der Klasse \texttt{HierarchicalDataTemplate} können Vorlagen erstellt werden, in denen definiert wird, wie die Objekte von \texttt{Projects} visuell präsentiert werden. 
Für jede Klasse, die \texttt{IProjectExplorerItem} implementiert, wurde deshalb eine solche Vorlage definiert.

Die visuelle Präsentation jedes Baumelementes erfolgt durch ein Icon und eine Textbox. \\
Die Icons wurden mithilfe der \texttt{Image}-Klasse eingebunden. Sie stammen von der "`Visual Studio Image Library"', die von Microsoft frei zur Verfügung gestellt wird \cite{VSImgLib}. \\
...
Der Wechsel von Modus 1 in Modus 2 soll entweder über einen zweimaligen Linksklick auf die Textbox oder über das Kontextmenü des Elementes möglich sein. 
WPF enthält keine Textbox, die die beschriebenen Anforderungen erfüllt. Es wurde deshalb das Paket \texttt{InplaceEditBoxLib} verwendet, das in Abschnitt \ref{Abstraktionsebene1} "`Abstraktionsebene 1"' beschrieben wurde. 
